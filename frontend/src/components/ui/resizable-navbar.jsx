...
export const NavBody = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-white px-4 py-2 lg:flex dark:bg-transparent",
    ...
    <motion.div
      ...
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-black dark:text-neutral-500"
          key={`link-${idx}`}
          href={item.link}>
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-gray-100 text-white dark:bg-neutral-800" />
          )}
...
export const NavbarLogo = ({ visible }) => {
  return (
    <a
      ...
      <span
        className={cn(
          "font-bold text-2xl transition-colors text-black"
        )}
      >
        PeerPledge
      </span>
